...
And just because we can, and to demonstrate that the coordinate systems of our created raster and our previously defined spatial vector object match, plot the spatial polygon `pols`and the spatial points `pts` on top of the raster (Tip: you can use `add=T` in the plot command to add additional layers on top of an existing plot without overwriting the previous plot).
