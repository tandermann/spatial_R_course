...
The data is available via the [Phylacine database](https://megapast2future.github.io/PHYLACINE_1.2/). Click on the link in the 'Download the data' section to start the download. Unzip the downloaded file and copy it into your data folder that you downloaded from the course GutHub repo. The raster data we're after is stored in the `Data_2/Ranges/Present_natural/` folder (Windows users remmeber to replace all `/` with `\\`). You can see all files that are present in that folder using the `list.files()` command:

```{r, message=FALSE, warning=FALSE}
species_ranges=list.files('../data/Data_2/Ranges/Present_natural/')
```
...
```{r, message=FALSE, warning=FALSE}
filepath = paste0('../data/Data_2/Ranges/Present_natural/',species_ranges[1])
...
merged_rasters = raster(paste0('../data/Data_2/Ranges/Present_natural/',species_ranges[1]))
for (i in species_ranges[2:length(species_ranges)]){
    species_raster=raster(paste0('../data/Data_2/Ranges/Present_natural/',i))
...
The present natural raster files are stored at `../data/Data_2/Ranges/Present_natural/`. To find the raster file of a specific species you can use a combination of the gsub command (which will replace the space with a '_' in the species name) and the paste0 command (which you can use to paste the .tif file ending to the file and to join the filename with the path to the folder):

```{r, message=FALSE, warning=FALSE}
species = 'Loxodonta africana'
filename = gsub(" ", "_", species)
file_path = paste0('../data/Data_2/Ranges/Present_natural/',filename,'.tif')
